...
<div class="part-container">
  <Modal v-model="openDialog" :title="isCreate ? '添加权限' : '修改权限'" :width="350">
    <Form :model="formUserRoles" :label-width="80" :rules="rules" ref="userRoles">
      ...
      <FormItem label="权限名称" prop="name">
        <Input v-model="formUserRoles.name" placeholtder="请输入添加权限" type="textarea" :autosize="{minRows: 3}"></Input>
      </FormItem>
      <FormItem label="权限描述" prop="desc">
...
export default class UserPartView extends Vue {
  ...
  private rules: object = {
    pid: [{ required: true, message: '请选择父级站点', trigger: 'change' }],
  ...
  private async getTableList(): Promise<any> {
    try {
      ...
      const [data, data1] = await getRoleList()
